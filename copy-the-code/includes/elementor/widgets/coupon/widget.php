...
class Coupon extends Widget_Base {
    ...
    public function get_title() {
        return esc_html__( 'Coupon Code', 'ctc' );
    }
    ...
    protected function _register_controls() {
        $this->start_controls_section( 'image_section', [
            'label' => esc_html__( 'Coupon Image', 'ctc' ),
        ] );
        ...
        $this->add_control( 'heading', [
            ...
            'default' => esc_html__( 'Up to 50% off', 'ctc' ),
        ] );
        $this->end_controls_section();
        $this->start_controls_section( 'description_section', [
            'label' => esc_html__( 'Short Description', 'ctc' ),
        ] );
        ...
        $this->start_controls_section( 'coupon_button_section', [
            'label' => esc_html__( 'Coupon Code', 'ctc' ),
        ] );
        $this->add_control( 'coupon_code', [
            'label'   => esc_html__( 'Coupon Code', 'ctc' ),
            'type'    => Controls_Manager::TEXT,
            'default' => esc_html__( 'CTC30', 'ctc' ),
        ] );
        $this->add_control( 'display_slide_button', [
            'label'   => esc_html__( 'Display Slide Button', 'ctc' ),
            'type'    => Controls_Manager::SWITCHER,
            'default' => 'yes',
        ] );
        // Button Text.
        $this->add_control( 'slide_button_text', [
            'label'     => esc_html__( 'Slide Button Text', 'ctc' ),
            'type'      => Controls_Manager::TEXT,
            'default'   => esc_html__( 'Show Coupon Code', 'ctc' ),
            'condition' => [
        ...
        $this->add_control( 'button_link', [
            'label'     => esc_html__( 'Button Link', 'ctc' ),
        ...
        $this->add_control( 'link_target', [
            'label'     => esc_html__( 'Link Target', 'ctc' ),
            'type'      => Controls_Manager::SELECT,
            'default'   => '_blank',
            'options'   => [
                '_self'  => esc_html__( 'Self', 'ctc' ),
                '_blank' => esc_html__( 'Blank', 'ctc' ),
            ],
            'condition' => [
                'display_slide_button' => 'yes',
            ],
        ...
        $this->start_controls_section( 'details_section', [
            'label' => esc_html__( 'Details', 'ctc' ),
        ] );
        // Enable Toggle Details.
        $this->add_control( 'toggle_details', [
            'label'   => esc_html__( 'Enable Toggle Details', 'ctc' ),
            'type'    => Controls_Manager::SWITCHER,
            'default' => 'yes',
        ] );
        // Details.
        ...
        $this->add_control( 'button_text', [
            'label'   => esc_html__( 'Copy Button Text', 'ctc' ),
            'type'    => Controls_Manager::TEXT,
            'default' => esc_html__( 'Copy Code', 'ctc' ),
        ] );
        $this->add_control( 'button_text_copied', [
            'label'   => esc_html__( 'After Copy Button Text', 'ctc' ),
            'type'    => Controls_Manager::TEXT,
            'default' => esc_html__( 'Code Copied', 'ctc' ),
        ] );
        // Show Icon.
